...
class FamilyExcusesFragment : BaseFragment() {


    private val binding by lazy {
        FragmentFamilyExcusesBinding.inflate(layoutInflater)
    }
    ...
    private fun observeData() {
        excusesViewModel.familyExcuses.observe(viewLifecycleOwner) { state ->
            ...
            when (state) {
                ...
                is UIState.ERROR -> {
                    ...
                    state.error.localizedMessage?.let {
                        showError(it) {
                            excusesViewModel.getFamilyExcuses()
                        }
                    }
                }
                else -> {}
